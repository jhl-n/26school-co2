...
import { Wind, Map as MapIcon, Plus, Trash2, Building2, Users, Edit3, X, AlertCircle, Navigation } from 'lucide-react';
...
/**
 * [Firebase 설정 및 오류 수정]
 * 1. appId 내부의 슬래시(/)를 언더바(_)로 치환하여 Firestore 경로 오류(세그먼트 개수 불일치)를 해결합니다.
 * 2. 모든 로직과 스타일을 하나로 통합하여 파일 참조 오류를 방지합니다.
 */
...
// [오류 수정] appId에 포함된 '/'가 경로를 쪼개지 않도록 '_'로 변환
      ...
      <div className="max-w-7xl mx-auto">
        <header className="mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 font-bold text-xs mt-0.5 flex items-center gap-2"><Users size={12}/> 실시간 탐사 공유</p>
            </div>
          </div>
          <div className="flex flex-wrap bg-white p-2 rounded-2xl shadow-sm border border-slate-200 gap-3">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
          <div className="xl:col-span-3 space-y-6">
            <div className="bg-white p-4 md:p-6 rounded-[2rem] shadow-xl relative aspect-[25/11] border-2 border-slate-100 overflow-hidden shadow-inner">
                {/* 층수 및 범례 오버레이 */}
                <div className="absolute top-4 left-4 z-20 flex flex-col gap-3">
                  <div className="bg-slate-900 text-white px-4 py-2 rounded-xl font-black text-sm shadow-xl flex items-center gap-2 border border-slate-700">
                    <MapIcon size={16} className="text-blue-400" />
                    {selectedFloor}층
                  </div>
                  
                  <div className="bg-white/90 backdrop-blur-md p-2 rounded-xl border border-slate-200 shadow-lg space-y-1 w-32 hidden md:block">
                ...
                {schoolStructure[selectedFloor].rooms.map(room => {
                  ...
                  return (
                    <button key={room.id} onClick={() => setSelectedRoom(room)} 
                      ...
                      // 버튼 및 글자 크기를 처음의 적절한 크기로 복구
                      className={`absolute transform -translate-x-1/2 -translate-y-1/2 w-16 md:w-24 h-9 md:h-11 border rounded-lg text-[9px] md:text-[11px] font-bold transition-all flex items-center justify-center px-1 text-center leading-tight
                        ${isSelected ? 'border-blue-600 ring-4 ring-blue-50 z-50 scale-110 shadow-xl' : 'bg-white border-slate-200 hover:border-blue-300 text-slate-700 shadow-sm'}`}>
                      {room.name}
                      {data && (
                        <div className="absolute -top-4 -right-4 z-40">
                          <div className={`${quality.color} text-white text-[8px] md:text-[10px] px-2 py-0.5 rounded-full font-black shadow-lg border-2 border-white animate-bounce flex flex-col items-center`}>
                            {data.ppm}
                          </div>
                        </div>
                      )}
            ...
            <div className={`bg-white p-5 rounded-2xl shadow-xl border-2 transition-all ${editingId ? 'border-orange-500 ring-4 ring-orange-50' : selectedRoom ? 'border-blue-500 ring-4 ring-blue-50' : 'border-slate-100'}`}>
               <div className="flex items-center justify-between mb-4">
                  <h3 className="font-black text-slate-700 text-sm flex items-center gap-2">
                    ...
                    {selectedRoom && <span className="text-blue-600 ml-1">[{selectedRoom.name}]</span>}
                  </h3>
                  {editingId && (
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end text-left">
                  <div className="space-y-1">
                    ...
                    <input type="date" value={measureDate} onChange={e => setMeasureDate(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-xs outline-none focus:border-blue-500" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 tracking-wider">탐사대원 성명</label>
                    <input type="text" value={studentName} onChange={e => setStudentName(e.target.value)} placeholder="이름" disabled={!selectedRoom} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-xs outline-none focus:border-blue-500" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 tracking-wider">농도 (PPM)</label>
                    <input type="number" value={newPpm} onChange={e => setNewPpm(e.target.value)} placeholder="0" disabled={!selectedRoom} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-black text-blue-600 text-sm outline-none focus:border-blue-500" />
                  </div>
                  <button onClick={handleSaveData} disabled={!selectedRoom || !newPpm || !studentName} className={`h-[40px] rounded-xl font-black text-white text-xs transition-all shadow-md ${(!selectedRoom || !newPpm || !studentName) ? 'bg-slate-200 shadow-none' : editingId ? 'bg-orange-500 hover:bg-orange-600' : 'bg-blue-600 hover:bg-blue-700 active:scale-95 shadow-blue-100'}`}>
          ...
          <div className="xl:col-span-1 bg-slate-900 rounded-[2rem] p-5 text-white overflow-hidden flex flex-col max-h-[85vh] shadow-2xl text-left border border-slate-800">
            <h2 className="font-black text-lg mb-6 flex justify-between items-center">
               <div className="flex items-center gap-2"><Building2 size={18} className="text-blue-400"/> 실시간 로그</div>
               <span className="text-[10px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full font-bold">{filteredData.length}건</span>
            </h2>
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-hide">
              {filteredData.length === 0 ? (
                ...
                [...filteredData].sort((a,b) => (b.createdAt || 0) - (a.createdAt || 0)).map(m => {
                  ...
                  return (
                    <div key={m.id} className={`p-4 rounded-xl border-l-4 relative group bg-slate-800/50 border-${q.color.split('-')[1]}-500 shadow-sm`}>
                      <div className="flex justify-between items-start mb-1">
                        <div className="text-[9px] font-black text-slate-500">{m.date} {m.timestamp}</div>
                        {isOwnRecord && (
                          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => startEdit(m)} className="text-blue-400 hover:text-blue-300" title="수정"><Edit3 size={10}/></button>
                            <button onClick={() => handleDelete(m.id)} className="text-rose-500 hover:text-rose-400" title="삭제"><Trash2 size={10}/></button>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <div className="flex items-center gap-1.5">
                          <span className="bg-slate-700 text-blue-300 text-[7px] px-1.5 py-0.5 rounded-md font-black">{m.classGroup}</span>
